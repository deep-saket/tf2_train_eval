...
def decode(labels, priors, variances=[0.1, 0.2]):
    """decoding"""
    bbox = _decode_bbox_np(labels[:, :4], priors, variances)
    landm = _decode_landm_np(labels[:, 4:14], priors, variances)
    landm_valid = labels[:, 14][:, None]
    conf = labels[:, 15][:, None]
...
def _decode_bbox_np(pre, priors, variances=[0.1, 0.2]):
    """Decode locations from predictions using priors to undo
    the encoding we did for offset regression at train time.
    Args:
        pre (tensor): location predictions for loc layers,
...
def _decode_landm_np(pre, priors, variances=[0.1, 0.2]):
    """Decode landm from predictions using priors to undo
    the encoding we did for offset regression at train time.
    Args:
        pre (tensor): landm predictions for loc layers,
...
def thresholded_indices(scores, score_threshold):
    return np.where(scores > score_threshold)[0].tolist()

def nms(dets, max_output_size, thresh):
    x1 = dets[:, 0]
    y1 = dets[:, 1]
    x2 = dets[:, 2]
    y2 = dets[:, 3]
